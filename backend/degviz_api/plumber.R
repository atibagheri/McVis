...
function(req, res) {
  ...
  cat("📄 file_path (from backend):", file, "\n")
  cat("🔧 mode:", mode, "\n")
  cat("🧬 species:", species, "\n")

  if (!file.exists(file)) {
    res$status <- 400
    return(list(success = FALSE, error = paste("File not found in container:", file)))
  }
  ...
  if (isFALSE(result$success %||% TRUE)) {
    res$status <- 500
    return(result)
  }
  ...
  list(
    ...
    unmapped_base64    = result$unmapped_base64,
